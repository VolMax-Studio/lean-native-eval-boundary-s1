...
**Current status: pre-gate · execution HALT · scientific verdict null · not frozen.** The latest formal Gate result is **FIXES-PENDING (F-N)** on the previous review package. Technical findings N-1 through N-4 are closed. The F-N clarification is present; this revision centralizes role assignments and uses role-based scientific rules. No Lean execution or full-denominator source scan has been performed in this task. [STATUS.md](STATUS.md) records the current state and remaining freeze requirements.
...
The designated Gate performs formal review; the final Ratifier ratifies the result. Concrete role holders and conflict disclosures are maintained only in [external/GOVERNANCE.md](external/GOVERNANCE.md). Creator self-checks and synthetic replay do not replace formal Gate review. The specification remains the same when a qualified person, model or institution takes over a role. The complete candidate freeze commit must receive its **own exact-SHA formal Gate review and separate Operator ratification**, as required by [the freeze-boundary rule](INSTANCE_RULES.md#freeze-boundary); a passing repair-package review does not cover future execution pins.
