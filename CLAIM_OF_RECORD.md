...
| R-B-affects: universal version coverage | T-B1 full-denominator syntactic relation | UNDERDETERMINED relative to behavioral affects; investigator-defined proxy | Verified / Not Verified / Not Demonstrated / Deferred for registered source proxy only |
| Investigator-defined T-B1: every denominator member has the registered signature pair | Frozen source_proxy.py on both pinned files for every member | EQUIVALENT to this explicitly syntactic investigator claim only | All applicable entries have signature pair, or identified proxy counterexample; no behavioral translation |
...
A failed structural matcher is not a scientific counterexample to the broader source claim. A confirmed proxy NO_MISMATCH is a counterexample only to the investigator-defined universal proxy proposition. The design has no verdict-bearing test of patch ancestry or universal behavioral coverage. Gate and Ratifier must preserve this scope in final wording.
